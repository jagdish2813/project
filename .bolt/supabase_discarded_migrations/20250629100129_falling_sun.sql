/*
  # Add Material Pricing and Quote Generation System

  1. New Tables
    - `designer_material_prices` - Store designer-specific material pricing
    - `designer_quotes` - Store quotes generated by designers
    - `quote_items` - Store individual items in quotes

  2. Security
    - Enable RLS on all tables
    - Add policies for designers to manage their materials and quotes
...
BEGIN
  -- Get the designer ID
  SELECT id INTO designer_id FROM designers WHERE email = 'priya.sharma@interiorcraft.com' LIMIT 1;
  
  IF designer_id IS NOT NULL THEN
    -- Check if material already exists
    SELECT EXISTS (
      SELECT 1 FROM designer_material_prices 
      WHERE designer_id = designer_id AND name = 'Marine Plywood'
    ...
    SELECT EXISTS (
      SELECT 1 FROM designer_material_prices 
      WHERE designer_id = designer_id AND name = 'High Pressure Laminate (HPL)'
    ) INTO material_exists;
    ...
    SELECT EXISTS (
      SELECT 1 FROM designer_material_prices 
      WHERE designer_id = designer_id AND name = 'Soft Close Hinges'
    ) INTO material_exists;
